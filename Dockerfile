...
RUN mkdir -p /usr/src/ama_hub

WORKDIR /usr/src/ama_hub
...
COPY . /usr/src/ama_hub

RUN chmod +x /usr/src/ama_hub/tasks.py \
    && chmod +x /usr/src/ama_hub/entrypoint.sh

# app-specific requirements
RUN pip install --upgrade --no-cache-dir --src /usr/src -r requirements.txt
RUN pip install --upgrade -e .

ENTRYPOINT ["/usr/src/ama_hub/entrypoint.sh"]